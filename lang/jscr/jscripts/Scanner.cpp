...
const int Buffer::EoF = COCO_WCHAR_MAX + 1;
...
void Scanner::Init() {
	...
	keywords.set(L"def", 7);
	keywords.set(L"case", 16);
	keywords.set(L"default", 19);
	keywords.set(L"break", 20);
	keywords.set(L"continue", 21);
	keywords.set(L"do", 22);
	keywords.set(L"while", 23);
	keywords.set(L"for", 24);
	keywords.set(L"if", 25);
	keywords.set(L"else", 26);
	keywords.set(L"return", 27);
	keywords.set(L"switch", 28);
...
Token* Scanner::NextToken() {
	...
	switch (state) {
		...
		case 2:
			...
			else if (ch == L'L' || ch == L'U' || ch == L'l' || ch == L'u') {AddCh(); goto case_4;}
		...
		case 6:
			...
			if ((ch >= L'0' && ch <= L'9') || (ch >= L'A' && ch <= L'F') || (ch >= L'a' && ch <= L'f')) {AddCh(); goto case_6;}
			else if (ch == L'L' || ch == L'U' || ch == L'l' || ch == L'u') {AddCh(); goto case_7;}
			else {t->kind = 3; break;}
		case 7:
			case_7:
			{t->kind = 3; break;}
		...
		case 9:
			...
			{t->kind = 4; break;}
		case 10:
			if (ch <= L'&' || (ch >= L'(' && ch <= L'[') || (ch >= L']' && ch <= 65535)) {AddCh(); goto case_11;}
		...
		case 13:
			...
			else if (ch == L'X' || ch == L'x') {AddCh(); goto case_5;}
			else {t->kind = 2; break;}
		case 14:
			case_14:
